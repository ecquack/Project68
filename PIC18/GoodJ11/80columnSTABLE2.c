...
#define SELF_INT      // define this if we do our own interrupt fast interrupt handling
...
#define  VSyncHChars    4           // duration of VSYNC pulse in 4 pixel units
   #ifdef COLUMN40
   #define  HBackPorch     21          // horizontal backporch in 4 pixel units
   #else
...
#org 0x500,0x1000
   ...
   static int heven=0,hodd=1;       // interlace mode- not fully implemented
   #locate hline=0x50
...
endpixels:
   ...
   else if(hline>=FIELDLINES-OVERSCANLINES)
   {
#asm
      BCF      LATC,SYNC_BIT     // output_low(SYNC_PIN);
      MOVLW    HSyncHChars
...
idletime:
      MOVLW    8        // number of instructions div 3 to pause
      MOVWF    hcount
idleloop:
      DECFSZ   hcount
      BRA      idleloop
...
#ifdef SELF_INT
unsigned int safety[0x15];
#locate safety=0x004

#INT_GLOBAL
void HardInterrupt(void)
{
 #asm
      NOP
      NOP
...
//   for(index=0;index<CONSOLECOLS*2;index++) console[index]=0; //wipe the first two lines
   ...
   for(index=0;index<CONSOLERAM;index+=CONSOLECOLS) // fill the rest of the screen with a test pattern
   {
...
#ifdef COLUMN40
   strcpy(&console[CONSOLECOLS/2-4],"EQUACK 40"); // centered at the top of the screen
#else
   strcpy(&console[CONSOLECOLS/2-4],"EQUACK 80"); // centered at the top of the screen
#endif
...
#endif      
      delay_fields(60); // wait for one second 
      ScrollUp(1);
   }
}
