...
#define  HBlankHChars   22    // change this to adjust the left margin (overscan area)
...
#INT_EXT2      // the keyboard take priority over the video scan so we don't lose any bits
...
int int_w,int_bsr,int_status,int_fsrl,int_fsrh,int_tabl,int_tabh,int_tabu;
...
#asm
   ...
   MOVFF    BSR,int_bsr
   MOVFF    FSR0L,int_fsrl
   MOVFF    FSR0H,int_fsrh
   MOVFF    TBLPTRL,int_tabl
   MOVFF    TBLPTRH,int_tabh
   MOVFF    TBLPTRU,int_tabu
#endasm
...
#asm
   MOVFF    int_fsrl,FSR0L
   MOVFF    int_fsrh,FSR0H
