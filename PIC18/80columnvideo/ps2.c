...
#define NEXTKEYDELAY 10    // minimum time in horizontal scan lines for keyboard scan code
...
//   static int windozed; // windows command key
   ...
   long scandex;
      ...
      if(ps2.state)// yes, there is a new scancode waiting
      {
         ...
         if(keydebug)
         {
            sprintf(shing,"Scancode %02X ASCII %02x RAWSCAN %02X",thiskey,out,ps2.code);
            PrintString(shing);
            PrintChar(13);
